// Import necessary dependencies from React and MUI library
...
const ProductView = () => {
    ...
    const [bottomSheetOpen, setBottomSheetOpen] = useState(false); // State for bottom sheet open (selecting size and quantity)
    ...
    const handleOpenCardView = () => {
        if (cart.length === 0) {
            ...
            setAlertSeverity('error'); // Set severity of Snackbar to 'error'
            setAlertOpen(true); // Open Snackbar
        } else {
            navigate('/product/card'); // Navigate to CardView route if cart is not empty
        }
    };
    ...
    // Function for handling opening bottom sheet (selecting size and quantity)
    ...
    // Function for handling closing bottom sheet
    ...
    const filterDrinks = (drinks) => {
        return drinks.filter(drink => {
            ...
            // Check if drink name contains search query (case-insensitive)
